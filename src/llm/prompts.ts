...
target_cards（多卡編輯）：
- 分析使用者訊息影響哪些卡片類型：
  - "world" - 世界層級設定（目標、規則、憲法）
  - "workflow" - 工作流定義（步驟、觸發條件）
  - "task" - 單次任務執行
  - "pipeline" - 排程工作流（時間表、步驟）
  - "adapter" - 平台整合（discord、telegram、x）
  - "commerce" - 定價與商品
  - "org" - 組織架構（部門、主管、權限）
- 如果訊息影響多個卡片，在 target_cards 列出所有相關類型
- 範例：「Safety Chief 應該有 rollback 權限並且每 15 分鐘巡邏一次」→ target_cards: ["org", "pipeline"]
- 如果不確定，不要加 target_cards（預設會更新所有現有卡片）
