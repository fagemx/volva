...
Migration required: check board.json schema compatibility
```
...
Uses Ably for pub/sub with horizontal scaling.
...
Uses Axum extractors for request validation.
```
...
✅ **Good (no release):**
```
refactor: simplify handler extraction in api routes

Reduces boilerplate by using shared extractors.
