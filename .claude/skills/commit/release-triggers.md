...
This document explains how conventional commit types trigger automated releases via release-please in the karvi project.

## Overview

The karvi project uses **release-please** for automated versioning and releases. Not all commit types trigger releases - understanding which commits create new versions is critical for proper release management.
...
feat: add sandbox execution
fix: resolve timeout in runner
```
...
BREAKING CHANGE: Session table column names have changed.
Migration script required if applicable.
```
...
- Changing function signatures in public crates
...
You're adding a new CLI command.
...
You're fixing a broken state transition.

**Commit:**
```
fix(core): correct run state machine transition
```
...
**Commit:**
```
refactor: rename session variables for consistency
```
